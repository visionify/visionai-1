...
class TestInvokeCliCamera(unittest.TestCase):
    ...
    def test_invoke_camera(self):
        output = invoke_cmd(f'python -m visionai camera')
        assert 'Error' in output
        assert 'Missing command' in output

    @WorkingDirectory(PKGDIR)
    def test_invoke_camera_help(self):
        output = invoke_cmd('python -m visionai camera --help')
    ...
    def test_invoke_camera_add_help(self):
        output = invoke_cmd('python -m visionai camera add --help')
        assert 'Usage' in output
    ...
    def test_invoke_camera_add_remove(self):
        ...
        output = invoke_cmd('python -m visionai camera remove --name TEST-999')
        ...
        # list camera
        output = invoke_cmd('python -m visionai camera list')
